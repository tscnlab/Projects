...
def check():
    ...
    try:
        with sync_playwright() as playwright:
            ...
            page.goto(origin+'/?q=engineering', wait_until='networkidle')
            assert page.locator('.project-card:visible').count() > 0
            ...
            routes = ['/','/about.html','/expectations.html','/funding.html','/apply.html','/programmes.html','/404.html'] + [f'/projects/{p["slug"]}/' for p in projects()]
            for width,height in [(1440,1000),(390,844),(320,740)]:
                ...
                for route in routes:
                    ...
                    if (width,route) in [(390,'/'),(390,'/projects/smartphone-corneal-irradiance/'),(1440,'/projects/darkness-dose/'),(1440,'/projects/schlaf-gesellschaft-unterricht/')]:
            ...
            for filename, heading in [('about.html','About the unit'),('expectations.html','General expectations'),('funding.html','Funding and remuneration')]:
                offline.goto((OUT / filename).as_uri())
                assert offline.locator('h1').inner_text() == heading
                offline.get_by_role('navigation').get_by_role('link',name='Projects',exact=True).click()
                assert offline.url == home_file
            offline.goto((OUT / 'programmes.html').as_uri())
            offline.locator('.programme-list a').filter(has_text='Engineering').first.click()
            assert offline.url.startswith(home_file + '?q=')
        ...
        assert not failures, '\n'.join(failures)
        report = {'result':'PASS','projects':count,'layout_checks':observations,'behaviour':['Search by hidden metadata','Search + location intersection','Empty state + reset','URL persistence','Keyboard skip link','PDF download','German language','No-JavaScript catalogue','Local HTML navigation and programme search'],'browser_errors':failures}
